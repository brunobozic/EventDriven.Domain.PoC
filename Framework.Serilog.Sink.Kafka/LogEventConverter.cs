...
namespace Elastic.CommonSchema.Serilog
{
    ...
    public static class LogEventConverter
    {
        ...
        private static Trace GetTrace(LogEvent logEvent)
        {
        ...
        private static IDictionary<string, object> GetMetadata(LogEvent logEvent,
            ISet<string> logEventPropertiesToFilter)
        {
        ...
        private static object PropertyValueToObject(LogEventPropertyValue propertyValue)
        {
            switch (propertyValue)
            {
                ...
                case DictionaryValue dv:
                    return dv.Elements.ToDictionary(kvp => ToSnakeCase(kvp.Key.Value.ToString()),
        ...
        private static Log GetLog(LogEvent e, IReadOnlyList<Exception> exceptions,
            IEcsTextFormatterConfiguration configuration)
        {
        ...
        private static Event GetEvent(LogEvent e)
        {
            var evnt = new Event
            {
                ...
                Id = e.TryGetScalarPropertyValue(SpecialKeys.ActionId, out var actionId)
                    ...
                    : null,
                Kind = e.TryGetScalarPropertyValue(SpecialKeys.ActionKind, out var actionKindProperty)
                    ? actionKindProperty.Value.ToString()
                    : null,
