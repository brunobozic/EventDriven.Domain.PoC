...
namespace EventDriven.Domain.PoC.Domain.DomainEntities.UserAggregate
{
    public class User : BasicDomainEntity<Guid>, IAuditTrail, IAggregateRoot
    {
            ...
            DateTimeOffset activeTo, string password, User creator, string origin)
        {
            ...
            var user = CreateUserInstance(userId, email, userName, firstName, lastName, oib, dateOfBirth, RegistrationStatusEnum.WaitingForVerification, isSeed: false);
            ...
            user.AddDomainEvent(new UserCreatedDomainEvent(userId, email, userName, firstName, lastName, oib, dateOfBirth, DateTime.UtcNow, user.EmailVerificationToken, creator?.Id, user.UserResourceId, origin, SharedKernel.EventTypeEnum.UserCreatedDomainEvent));
            return user;
        }
            ...
            DateTimeOffset activeTo, string password, User activator, string origin, bool isSeed)
        {
            ...
            user.AddDomainEvent(new UserCreatedDomainEvent(userId, email, userName, firstName, lastName, oib, dateOfBirth, DateTime.UtcNow, user.EmailVerificationToken, activator?.Id, user.UserResourceId, origin, SharedKernel.EventTypeEnum.UserCreatedDomainEvent));
            return user;
        }
            ...
            string password, string role, User creator, string origin)
        {
            ...
            var user = CreateUserInstance(userId, email, userName, firstName, lastName, "", DateTimeOffset.MinValue, RegistrationStatusEnum.WaitingForVerification, isSeed: false);
            user.AddPasswordHash(password);
            user.AddVerificationToken(RandomStringHelper.RandomTokenString());

            user.AddDomainEvent(new UserCreatedDomainEvent(userId, email, userName, firstName, lastName, "", DateTimeOffset.MinValue, DateTimeOffset.MinValue, "", creator.Id, user.UserResourceId, origin, SharedKernel.EventTypeEnum.UserCreatedDomainEvent));
            return user;
        }
        ...
        public void AccountActivationMailNotSent(string eMessage)
        {
            ...
            {
                Log.Information("Attempting to send account activation mail for user {UserId}, {Email}", this.Id, this.Email);
        ...
        public bool Activate(DateTimeOffset from, DateTimeOffset to, User activatedBy)
        {
            ...
            Log.Information("Activating user {UserId}", this.Id);
        ...
        public bool AddPasswordHash(string password)
        {
            ...
            Log.Information("Adding password hash for user {UserId}", this.Id);
        ...
        public bool AddRefreshToken(RefreshToken.RefreshToken refreshToken)
        {
            ...
            Log.Information("Adding refresh token for user {UserId}", this.Id);
        ...
        public bool AddRole(Role role, User roleGiver)
        {
            using (LogContext.PushProperty("Method", nameof(AddRole)))
                ...
                try
                {
                    ...
                    Log.Information("Role {RoleName} added to User {UserId}", role.Name, this.Id);

                    return true;
                }
                catch (Exception ex)
                {
                    Log.Error(ex, "Error adding role {RoleName} to User {UserId}", role.Name, this.Id);
        ...
        public bool AddVerificationToken(string verificationToken)
        {
            ...
            Log.Information("Adding verification token for user {UserId}", this.Id);
        ...
        public bool AssignAddress(Address address, AddressType addressType, User addressAssigner)
        {
            ...
            Log.Information("Assigning address to user {UserId}, {Email}", this.Id, this.Email);
        ...
        public bool CreatePasswordResetToken(string randomToken)
        {
            ...
            Log.Information("Creating password reset token for user {UserId}", this.Id);
        ...
        public bool Deactivate(DateTimeOffset from, DateTimeOffset to, User deactivatedBy, string reason)
        {
            ...
            Log.Information("Deactivating user {UserId} for reason: {Reason}", this.Id, reason);
        ...
        public List<RefreshToken.RefreshToken> GetRefreshTokens()
        {
            ...
            Log.Information("Fetching refresh tokens for user {UserId}", this.Id);
        ...
        public bool OwnsToken(string refreshToken)
        {
            ...
            Log.Information("Checking if user {UserId} owns token {Token}", this.Id, refreshToken);
        ...
        public bool RemoveAddressFromUser(Address addressToRemove, AddressType addressType, User addressRemover)
        {
            ...
            Log.Information("Removing address from user {UserId}, {Email}", this.Id, this.Email);
        ...
        public bool RemoveRole(Role role, User removerUser)
        {
            using (LogContext.PushProperty("Method", nameof(RemoveRole)))
                ...
                try
                {
                    ...
                    Log.Information("Role {RoleName} removed from User {UserId}", role.Name, this.Id);

                    return true;
                }
                catch (Exception ex)
                {
                    Log.Error(ex, "Error removing role {RoleName} from User {UserId}", role.Name, this.Id);
        ...
        public bool RemoveStaleRefreshTokens()
        {
            ...
            {
                Log.Information("Removing stale refresh tokens for user {UserId}", this.Id);
        ...
        public bool RevokeToken(string token, string ipAddress, User revokerUser)
        {
            ...
            {
                Log.Information("Revoking token for user {UserId}", this.Id);

                try
                {
                    var refreshTokenDomain = _refreshTokens.SingleOrDefault(tok => string.Equals(tok.Token.Trim(), token.Trim(), StringComparison.CurrentCultureIgnoreCase));
        ...
        public void SetAccountActivationMailResent()
        {
            using (var scope = LogContext.PushProperty("Method", nameof(SetAccountActivationMailResent)))
            {
                ...
                try
                {
                    ...
                    Log.Information("Account activation mail resent for user {UserId}", this.Id);
                }
                catch (Exception ex)
                {
        ...
        public string SetEmailIsVerified()
        {
            ...
            try
            {
                if (TheUserIsInActiveState() && !IsDeleted && VerificationTokenExpirationDate >= DateTime.UtcNow)
                {
                    ...
                    Log.Information("Email verified for user {UserId}", this.Id);
                ...
                Log.Warning("Failed to verify email for user {UserId}. Reason: {Reason}", this.Id, friendlyErrorResponse);
                return friendlyErrorResponse;
            }
            catch (Exception ex)
            {
        ...
        public bool VerifyPasswordResetTokenThenResetPassword(string tokenReceivedByEmail, string newPassword)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(tokenReceivedByEmail))
                    throw new DomainException($"The token received by email must not be null. Unable to confirm password reset for user [ {UserName} ], email [ {Email} ].");
                if (string.IsNullOrEmpty(newPassword))
                    throw new DomainException($"The new password must not be null. Unable to confirm password reset for user [ {UserName} ], email [ {Email} ].");

                Log.Information("Verifying password reset token for user {UserId}", this.Id);
        ...
        private void GuardAgainstInactiveOrUnverifiedOrDeletedStatus()
        {
            if (!TheUserIsInActiveState() || !TheUserHadBeenVerified() || IsDeleted)
            {
                var errorMsg = $"The user [{UserName} / {Email}] is either deactivated, deleted or has not yet verified his account.";
            ...
            using (LogContext.PushProperty("Method", nameof(UpdatePasswordThenRemoveResetToken)))
                ...
                try
                {
                    ...
                    if (VerificationTokenHasNotExpired())
                    {
                        ...
                        Log.Information("Password updated for User {UserId}", this.Id);
                    }
                    else
                    {
                ...
                {
                    Log.Error(ex, "Error updating password for User {UserId}", this.Id);
