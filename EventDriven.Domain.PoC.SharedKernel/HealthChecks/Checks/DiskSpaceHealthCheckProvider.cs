...
namespace EventDriven.Domain.PoC.SharedKernel.HealthChecks.Checks
{
    ...
    public class DiskSpaceHealthCheckProvider : IHealthCheckProvider
    {
        ...
        {
            var result = new HealthCheckItemResult(nameof(DiskSpaceHealthCheckProvider), SortOrder, "Checks disk space",
            ...
            try
            {
                ...
                result.Messages.Add(
                    $"There is {(percentageFree > MinPercentageFree ? "more" : "LESS")} than {MinPercentageFree} percent available disk space.");
            }
            catch
            {
