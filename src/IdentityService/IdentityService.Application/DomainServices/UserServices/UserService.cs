...
public class UserService : Service<User>, IUserService
{
    ...
    public async Task<AuthenticateResponse> AuthenticateAsync(AuthenticateRequest model, string ipAddress)
    {
        ...
        var applicationUser = await Repository.Queryable()
            .SingleOrDefaultAsync(x => x.Email.ToUpper().Trim() == model.Email.ToUpper().Trim());
        ...
        if (applicationUser == null ||
            ...
            throw new AppException("Email or password is incorrect, or the user has not verified his account [ " +
                                   model.Email.ToUpper().Trim() + " ]");
        ...
        catch (Exception ex)
        {
            ...
            returnValue.UserFriendlyMessage =
                "Problem logging in, generating a jwt token or generating a refresh token.";
