...
public class KafkaPollJobController : IJobController
{
    ...
    public void ReadAndProcessKafkaMessage(JobDataMap jobDataMap)
    {
        ...
        var resultOfPersisting = new PersistingResult();

        if (_c.Instance().Assignment.Count > 0)
            Log.Information("Topic: [ {KafkaConsumerTopic} ]" + ", partition: " +
                            _c.Instance().Assignment[0].Partition);
        ...
        var delay = Backoff.ExponentialBackoff(
            TimeSpan.FromMilliseconds(_settings.PollySettings.KafkaSharedExponentialBackoff.InitialDelayMiliseconds)
        ...
        var retryPolicy = Policy
            .HandleResult<ConsumeMessageResult>(r =>
                !r.Success && !r.ErrorMessage.Contains("No new messages") &&
                !r.ErrorMessage.Contains("No offset stored"))
            .WaitAndRetry(delay, (response, retryCount) =>
            {
                ...
                else // not enough info to do anything but log
                {
                    Log.ForContext("Retry", retryCount)
                        .ForContext("ErrorMessage", response.Result.ErrorMessage)
                        .Warning(
                            " => retry **failed**, Offset: [ {KafkaOffset} ], topic: [ {KafkaConsumerTopic} ], retry reason: {ErrorMessage}");
                }
            });
        ...
        else // Message was **NOT** read from kafka!!! unfortunately, this might as well mean that we get no information on what the current offset might be
        {
            ...
            Log.ForContext("Offset", kafkaMessage.Offset)
                .ForContext("Partition", kafkaMessage.Partition)
                .Information(
                    "Message **not** read from offset: [ {Offset} ], partition [ {Partition} ], topic: [ {KafkaConsumerTopic} ], found [ " +
                    _c.Instance().Assignment.Count + " ] assignments, brokers: [ {KafkaConsumerBootstrapServer} ]");
            ...
            if (_c.Instance().Assignment.Count > 0)
            {
                if (!kafkaMessage.ErrorMessage.Contains("Local: No offset stored") &&
                    !kafkaMessage.ErrorMessage.Contains("No new messages"))
                {
                    ...
                    Log.ForContext("Partition", kafkaMessage.Partition)
                        ...
                        .Information(
                            "Message at offset: [ {Offset} ], partition: [ {Partition} ], topic: [ {Topic} ] will be skipped, sending to dead letter queue, brokers: [ {KafkaConsumerBootstrapServer} ]");
                    ...
                    Log.ForContext("Partition", kafkaMessage.Partition)
                        ...
                        .ForContext("Offset", kafkaMessage.Offset)
                        .Warning(
                            "Message at offset: [ {Offset} ], partition: [ {Partition} ], topic: [ {Topic} ]  sent to dead letter queue, brokers: [ {KafkaConsumerBootstrapServer} ]");
    ...
    private void SkipProblematic(ConsumeMessageResult kafkaMessage)
    {
        try
        {
            _c.SkipPoisonPill(new TopicPartition(_c.GetTopic(), kafkaMessage.Partition),
                _c.Instance().Position(new TopicPartition(_c.GetTopic(), kafkaMessage.Partition)).Value);
        }
        catch (Exception skippingException)
        {
            Log
                ...
                .ForContext("Partition", kafkaMessage.Partition)
                .Fatal(
                    "Message **not** skipped, offset: [ {Offset} ], partition [ {Partition} ], topic: [ {KafkaConsumerTopic} ], found [ " +
                    _c.Instance().Assignment.Count + " ] assignments, brokers: [ {KafkaConsumerBootstrapServer} ]",
                    skippingException);
        }
    }
    ...
    private void NotifyProblemWith(ConsumeMessageResult consumedMessage)
    {
        ...
        Log.Error(readStatusMessage + ", at offset: [ {KafkaOffset} ]" +
                  ", Broker: [ {KafkaConsumerBootstrapServer} ]");
    }

    private void RereadTheSame(ConsumeMessageResult message)
    {
        var topicPartition = new TopicPartition(_c.GetTopic(), new Partition(message.Partition));
        _c.Instance().Assign(new List<TopicPartitionOffset> { new(topicPartition, message.Offset) });

        Log.ForContext("Offset", message.Offset)
            .ForContext("Partition", message.Partition)
            .Warning("Rereading the message, setting offset position to: [ {Offset} ], partition: [ {Partition} ] " +
                     ", Broker: [ {KafkaConsumerBootstrapServer} ]");
    }
