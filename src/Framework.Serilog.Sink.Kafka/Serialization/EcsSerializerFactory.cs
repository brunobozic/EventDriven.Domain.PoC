...
/// </summary>
/// <remarks>
///     This class should only be used for advanced use cases, for simpler use cases you can utilise the
///     <see cref="Base.Metadata" /> property.
/// </remarks>
/// <typeparam name="TBase">Type of the <see cref="Base" /> subclass</typeparam>
public static class EcsSerializerFactory<TBase>
{
    public static ValueTask<TBase> DeserializeAsync(Stream stream, CancellationToken ctx = default)
    {
        return JsonSerializer.DeserializeAsync<TBase>(stream, JsonConfiguration.SerializerOptions, ctx);
    }

    public static TBase Deserialize(string json)
    {
        return JsonSerializer.Deserialize<TBase>(json, JsonConfiguration.SerializerOptions);
    }

    public static TBase Deserialize(ReadOnlySpan<byte> json)
    {
        return JsonSerializer.Deserialize<TBase>(json, JsonConfiguration.SerializerOptions);
    }
