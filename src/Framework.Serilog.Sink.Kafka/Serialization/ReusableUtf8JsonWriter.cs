...
internal sealed class ReusableUtf8JsonWriter
{
    ...
    {
        var writer = Interlocked.Exchange(ref _cachedJsonWriter, null);
        return new ReusableJsonWriter(this, writer, text);
    }

    public ReusableJsonWriter NewJsonWriter(StringBuilder text)
    {
        return new ReusableJsonWriter(this, new Utf8JsonWriter(new MemoryStream()), text);
    }
    ...
    private void Return(Utf8JsonWriter writer, StringBuilder output)
    {
        ...
        Interlocked.Exchange(ref _cachedJsonWriter, writer);
    }

    private static void CopyToStringBuilder(ArraySegment<byte> byteArray, char[] encodingBuffer,
        StringBuilder output)
    {
        for (var i = 0; i < byteArray.Count; i += encodingBuffer.Length)
        {
            ...
            var charCount =
                Encoding.UTF8.GetChars(byteArray.Array, byteArray.Offset + i, byteCount, encodingBuffer, 0);
