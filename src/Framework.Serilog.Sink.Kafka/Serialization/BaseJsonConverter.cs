...
internal partial class BaseJsonConverter<TBase> where TBase : Base, new()
{
    ...
    private static void WriteMessage(Utf8JsonWriter writer, Base value)
    {
        writer.WriteString("message", value.Message);
    }

    private static void WriteLogLevel(Utf8JsonWriter writer, Base value)
    {
    ...
    private static void WriteTimestamp(Utf8JsonWriter writer, Base value)
    {
        ...
        if (value.Timestamp.HasValue)
            JsonConfiguration.DateTimeOffsetConverter.Write(writer, value.Timestamp.Value,
                JsonConfiguration.SerializerOptions);
        else writer.WriteNullValue();
    }
}

internal class BaseJsonConverter : BaseJsonConverter<Base>
{
}